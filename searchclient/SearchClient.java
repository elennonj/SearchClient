...
public class SearchClient {
	...
	public SearchClient(BufferedReader serverMessages) throws Exception {
		...
		// Loop that gets all of the lines into levelText
		while(!line.equals("")){
			levelText.add(line);
		...
		// Must find max number of items in each row to get num columns, not just the first row
		int numCols = levelText.get(0).length();
		for(int i = 1; i < levelText.size(); i++){
			numCols = Math.max(numCols, levelText.get(i).length());
		}
		int numRows = levelText.size() - 1;

		int row = 0;
		boolean agentFound = false;
		this.initialState = new Node(null, numRows, numCols);

		int index = 0;
		String currentLine = levelText.get(index);
		while (!currentLine.equals("")) {
			...
			index++;
			currentLine = levelText.get(index);
			row++;
		}
	}
